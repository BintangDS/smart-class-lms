...
<div class="space-y-10">
    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-12">
        ...
        <!-- Sidebar: Tugas Mendatang (4 cols) -->
        <div class="lg:col-span-4 space-y-4">
            <div class="rounded-2xl border border-slate-800 bg-slate-900/40 p-5 backdrop-blur space-y-4">
                <h2 class="text-lg font-extrabold text-white flex items-center gap-2">
                    <span class="inline-block h-2.5 w-2.5 rounded-full bg-violet-500 shadow-lg shadow-violet-500/30"></span>
                    Tugas Mendatang
                </h2>
                ...
                <div class="space-y-3 max-h-[600px] overflow-y-auto pr-1">
                    ...
                    @php
                        ...
                        $dueDateFormatted = \Carbon\Carbon::parse($assignment->due_date)->translatedFormat('d M Y, H:i');
                    @endphp
                    <div class="rounded-xl border border-slate-800/80 bg-slate-950/40 p-3.5 space-y-3 hover:border-slate-700 transition">
                        <div>
                            <span class="text-[10px] font-semibold text-indigo-400 uppercase tracking-wider block mb-0.5">
                        ...
                        <div class="flex items-center justify-between text-xs pt-1 border-t border-slate-900">
                            <div class="flex flex-col">
                                <span class="text-[9px] text-slate-500 uppercase tracking-wider">Batas Waktu</span>
                                <span class="font-medium {{ $isOverdue && !$sub ? 'text-rose-400' : 'text-slate-400' }}">
                                    {{ $dueDateFormatted }}
                                </span>
                            </div>
                            ...
                            <div>
                                @if($sub)
                                    @if($sub->score !== null)
                                        <span class="inline-flex items-center gap-1 rounded bg-emerald-500/10 px-1.5 py-0.5 text-[9px] font-bold text-emerald-400 border border-emerald-500/20">
                                            NILAI: {{ $sub->score }}
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 rounded bg-indigo-500/10 px-1.5 py-0.5 text-[9px] font-bold text-indigo-400 border border-indigo-500/20">
                                            DIKUMPUL
                                        </span>
                                    @endif
                                @elseif($isOverdue)
                                    <span class="inline-flex items-center gap-1 rounded bg-rose-500/10 px-1.5 py-0.5 text-[9px] font-bold text-rose-400 border border-rose-500/20">
                                        TERLEWAT
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 rounded bg-amber-500/10 px-1.5 py-0.5 text-[9px] font-bold text-amber-400 border border-amber-500/20">
                                        BELUM KUMPUL
                                    </span>
                                @endif
                            </div>
                    ...
                    @empty
                    <div class="rounded-xl border border-slate-800 bg-slate-950/20 p-6 text-center text-slate-500 text-xs">
                        Tidak ada tugas mendatang dari kelas yang kamu ikuti.
                    </div>
                    @endforelse
                </div>
            </div>
