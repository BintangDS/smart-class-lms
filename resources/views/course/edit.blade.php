...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-extrabold text-white">Edit Kursus</h1>
    </div>
    ...
    <div class="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-8 shadow-2xl backdrop-blur-xl">
        <form action="{{ route('courses.update', $course->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" required value="{{ old('title', $course->title) }}" placeholder="Contoh: Belajar Laravel 11 untuk Pemula"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="5" required placeholder="Tulis deskripsi detail materi kelas di sini..."
                    class="mt-1 block w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-slate-100 placeholder-slate-500 shadow-inner outline-none transition duration-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm">{{ old('description', $course->description) }}</textarea>
                @error('description')
                    <p class="mt-2 text-xs text-rose-400 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select id="status" name="status" required
                    class="mt-1 block w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-slate-100 placeholder-slate-500 shadow-inner outline-none transition duration-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm">
                    <option value="draft" {{ old('status', $course->status) == 'draft' ? 'selected' : '' }}>Draft (Sembunyikan dari Katalog)</option>
                    <option value="published" {{ old('status', $course->status) == 'published' ? 'selected' : '' }}>Published (Tampilkan di Katalog Siswa)</option>
                </select>
                @error('status')
                    <p class="mt-2 text-xs text-rose-400 font-medium">{{ $message }}</p>
                @enderror
